...
def get_element(dataset: str, class_name: str) -> Union[str, None]:
    '''getting the path to the item'''
    for file_name in os.listdir(os.path.join(dataset, "dataset", class_name)):
        yield file_name
...
def create_randomname_file(dataset: str, annotation_name: str, dir_copy: str) -> None:
    ...
    counter = 1
    create_dir(dir_copy)
    for dataset_class in os.listdir(os.path.join(dataset, "dataset")):
        for file_name in get_element(dataset, dataset_class):
            shutil.copy(os.path.join(os.path.join(dataset, "dataset", dataset_class), file_name),
